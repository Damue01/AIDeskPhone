...
<body>
      ...
      <section id="globalConfigPage" class="settings-page" hidden>
        <section id="globalConfigSection" class="settings-card global-settings-card" aria-labelledby="globalSettingsTitle">
          ...
          <div class="settings-row">
            ...
            <div class="row-label"><strong>语音 API Key</strong><span>ASR / TTS</span></div>
          ...
          <div class="settings-row">
            <span class="icon-cell"><i data-lucide="shield-keyhole"></i></span>
            <div class="row-label"><strong>Ark API Key</strong><span>通讯员 / 角色回复</span></div>
            <div class="row-control read-only-control">
              <input id="arkApiKey" class="secret-field" type="password" autocomplete="off" placeholder="保存后写入本机 .env">
              <button class="small-action" type="button" onclick="saveConfig()">保存</button>
            </div>
          </div>
  ...
  <script>
    ...
    function updateSpeechSecretState(status = {}) {
      ...
      const arkInput = $("arkApiKey");
      const arkConfigured = status.operator_credential_mode && status.operator_credential_mode !== "missing";
      if (arkInput) {
        arkInput.classList.toggle("configured", Boolean(arkConfigured));
        arkInput.placeholder = arkConfigured ? "••••••••••••（已保存）" : "保存后写入本机 .env";
      }
    }
    ...
    async function saveSpeechConfigIfNeeded() {
      const apiKey = $("speechApiKey")?.value?.trim() || "";
      const arkApiKey = $("arkApiKey")?.value?.trim() || "";
      const speaker = $("speechSpeaker").value.trim();
      const payload = {
      ...
      if (arkApiKey) payload.ark_api_key = arkApiKey;
      const result = await fetchJson("/api/speech/config", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
