...
| 3.3V/稳压模块 | 小电源板，端子常见丝印为 `VIN+`、`GND`、`VOUT+`、`GND` | 后续可能用于供电，但使用前必须实测输出电压 | `IMG_8615..JPG` |
...
不要默认用 ESP32-C3 的 3.3V 给 CSR8645 蓝牙音频模块供电。当前硬件现象显示 3.3V 可能偏低，可能导致不开机、配对不稳定、Windows 看不到麦克风、MIC 偏置异常或设备反复消失。

建议规则：

1. 优先按模块 `BAT+ / BAT-` 标称供电范围处理。
2. 多数 CSR8645 耳机小板按单节锂电池工作区间设计，常见是 3.7V 标称、4.2V 满电。
3. 3.3V 只能当作边缘情况看待，不建议作为最终供电方案。
4. 除非这块模块明确有 5V 输入或充电管理输入，否则不要把 5V 直接接到 `BAT+`。
5. 接听筒麦克风前先测：
...
### 8.5 CSR8645 用 3.3V 不稳定

优先按 5.1 的供电规则处理，把它当成供电问题排查。不要把 ESP32-C3 3.3V 作为最终 CSR8645 供电方案，除非这块具体模块已经被验证稳定。
