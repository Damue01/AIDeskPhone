...
这份文档说明 AI Desk Phone 当前主线要做成什么。它不是历史开发记录，也不是硬件焊接教程；硬件细节看 [HG113 连接方式](HG113_CONNECTION_MANUAL.md)，实际制作步骤看 [制作与维护手册](BUILD_MANUAL.md)。
...
挂机不会取消已经提交的后台任务。Agent 播报中挂机会暂停当前回话并重新进入回拨，用户再摘机后继续播报。
...
约 90 秒无人接听后，普通回拨会切换为忙音节奏。页面和日志保留完整调试信息，但电话里只播报整理后的结果。
...
- 管理 Agent session、prompt、tools、skills 和历史消息。
- 手动测试蜂鸣器、LED、模拟摘机/挂机和 hook。

地球指挥中心：`http://127.0.0.1:8765/command-center/`

- 作为电话待命主屏。
- 展示当前阶段：等待命令、接收指令、执行命令、等待接听、播报结果。
...
这只是当前样机记录，不是所有电话的通用保证。实际制作必须用万用表确认触点、线序、供电和绝缘。
