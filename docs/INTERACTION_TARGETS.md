...
2. ASR 开始实时识别。
3. 用户说出任务。
4. 用户停顿或挂机后，当前语音提交成一轮 user turn。
5. Agent 在后台处理、调用工具、生成电话回话。
6. 如果用户已经挂机，处理不能被取消；完成后蜂鸣器和 LED 提醒用户接听。
...
输入法模式适合 Codex、编辑器、聊天框或其他已经负责执行任务的第三方应用。
...
6. 第三方 AI 应用处理任务。
7. 第三方应用完成后调用本地 hook。
8. 控制台把 hook 文本整理成通讯员回话并加入回拨队列。配置了 `ARK_API_KEY` 时先让 Ark 角色模型润色；不可用时保留原文。
9. 电话响铃/亮灯。用户摘机后按队列播放回话。
10. 输入法模式下，如果用户在播报中挂机，当前播放停止。
...
3. 用户停顿、达到稳定文本、达到最长录音时间，或挂机后，当前语音提交。
4. Agent 可以先快速确认，也可以直接进入后台任务。
5. 挂机只关闭麦克风和听筒，不取消已经提交的后台任务。
6. 任务完成后，控制台把结果加入回话队列，并进入 LED/蜂鸣器回拨提醒。
7. 用户摘机后，TTS 播报通讯员回话。
8. Agent 播报中挂机时，当前回话暂停并重新放回回拨流程；用户再摘机后继续播报。
9. 用户在提交前说出明确取消意图，例如“取消”“不用了”“撤回”，应取消当前语音轮次，不提交任务。
...
7. 电话 Agent 角色模型根据用户文本、工具结果和最近历史生成回话。
8. 回话写入 assistant 文本消息，并进入既有回话队列/回拨流程。
9. 当上下文变长时，旧消息会被压缩成结构化摘要，最近消息保留原文。
...
不要从全局 PI、用户目录 `.agents` 或外部插件目录加载 skills。本项目不需要那一大批全局能力；后续需要新能力时，应在仓库内新增或修改 `.pi/skills`，让行为可审计、可提交、可复现。
...
项目名、产品名、Codex、HG113、地名和用户自定义词应尽量放入语音服务的热词/boosting table。当前 `.env.example` 支持：
