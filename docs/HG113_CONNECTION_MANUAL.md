...
本文记录当前已经跑通的 HG113 电话改造接线方式。当前方案不是蓝牙键盘方案，也不是 ESP32 自己托管网页；ESP32-C3 只负责采集状态、通过 Wi-Fi 发数据、接收电脑端命令并驱动蜂鸣器/LED。网页、波形和按钮控制都由电脑端本地控制台完成。
...
| 摘挂机检测地线 | GND | 六脚簧片/叉簧开关 2 号脚 | 当前实接按最新记录为 2-6 这一对触点 |
| 蜂鸣器 | GPIO21 | 蜂鸣器信号/正端 | 当前已通过网页和 UDP 命令测试可响 |
| 蜂鸣器地线 | GND | 蜂鸣器负端 | 必须与 ESP32-C3 共地 |
| LED | GPIO20 | LED 信号/正端 | 固件中蜂鸣器响时会同步点亮 LED |
| LED 地线 | GND | LED 负端 | 裸 LED 需要串联限流电阻 |
| ESP32-C3 供电 | USB-C | 电脑 USB | 当前主供电方式 |
...
如果以后机械方向和显示名称反了，优先在固件/网页里反转逻辑，不建议临时乱换线。
...
之前出现“一接上就一直 LOW / 被 3.3V 拉起来”的根因，就是原电话板和 ESP32 的上拉、电源状态互相影响。现在的原则是：开关只作为普通机械触点，一端 GPIO，一端 GND，不把电话原板电源混进去。
...
2. 如果是裸蜂鸣片、线圈、原电话大铃器，GPIO 可能推不动，后续应加三极管或驱动模块。
3. 当前已经听到蜂鸣器响，说明现有蜂鸣器接法在测试条件下可用。
