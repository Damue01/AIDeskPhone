...
如果你正在使用 Agent 模式，电话语音输入、ASR、工具调用和回复生成已经由本地控制台处理；hook 主要用于第三方任务完成提醒。
...
- 输入法模式下，播报中挂机会停止当前播放。
- Agent 模式下，播报中挂机会暂停当前回报并重新进入回拨；用户再摘机后继续播报。
- 用户语音已经提交后再挂机，不会取消后台处理；处理完成后会回拨。
...
如果控制台在线，脚本会读取当前回拨开关状态，然后把完成消息发送到本地 hook 地址。控制台会把消息加入回话队列，并启动提醒。
...
Codex 的 `Stop` 事件会在当前回合结束时调用 hook。首次启用或修改 hook 后，在 Codex 里用 `/hooks` 查看并信任这条项目 hook。

信任后，Codex 完成当前回合时会调用：
