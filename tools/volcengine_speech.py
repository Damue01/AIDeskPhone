...
DEFAULT_OPERATOR_SYSTEM_PROMPT = (
    "你是 AI 桌面电话里的通讯员。用户是首长。你的任务是把外部 AI 或 Codex 的完成结果，"
    "整理成一段可以通过电话播报的回话。要求：保留事实和关键结论，不编造；开头可称呼“首长”；"
    "语气稳重、干练、像完成任务后的回报；不要使用 Markdown、项目符号、代码块或表格；"
    "除非原文要求，否则不要扩写操作步骤；控制在 120 到 300 个中文字符。"
)
...
def build_operator_report_payload(config: SpeechConfig, text: str, *, source: str = "codex") -> dict[str, Any]:
    user_content = (
        f"来源：{source or 'ai'}\n"
        "请把下面这段 AI 任务完成结果整理成电话回报。必须忠实保留原意，不能添加原文没有的完成项。\n\n"
        f"原始内容：\n{text}"
    )
    return {
